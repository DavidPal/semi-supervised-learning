...
ERM needs significantly less labeled examples to learn for some unlabeled
distributions, e.g. unlabeled distribution is concentrated on a single point. If
ERM, Hanneke's algorithm, or some other distribution-independent algorithm had
sample complexity that matches (or nearly matches) the best known
distribution-specific bound for \emph{every} distribution, we could conclude
that the knowledge of unlabeled data distribution is completely useless.
...
\emph{any} distribution-independent algorithm for learning the class of projections
over $\{0,1\}^n$ requires, for some unlabeled distributions, $\Omega(\frac{\log
n}{\log \log n})$ times many labeled examples as Benedek-Itai bound.
...
We lower bound $K$. For any $y \in \{0,1\}^m$ and any $i,j \in \{1,2,\dots,n\}$,
$$
...
a sum of $n-1$ Bernoulli variables, one for each column except $i$.
...
Consider the class $C_n$ of monotone disjunctions over $D_n = \{0,1\}^n$. There
are $2^n$ functions in $C_n$. For each subset $I \subseteq \{1,2,\dots,n\}$
there is monotone disjunction $c_I:D_n \to \{0,1\}$ defined by
$$

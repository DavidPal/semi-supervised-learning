...
Let $X_1, X_2, \dots, X_n$ be i.i.d. that lie in interval $[a,b]$ with probability one.
...
some $i$, considers a random vector $X = (X[1], X[2], \dots, X[n])$ drawn from
...
Using Chernoff-Heoffding bound with $\epsilon = \frac{1}{2} - 2p(1-p)$,
$$
...
learning a target under a distribution from $\P_{n,p}$ the number
of labeled examples required is at least $\Omega(\log n)$.
...
\P_{n,p} \times C_n$ by probabilistic method.

Let $I$ be chosen uniformly at from $\{1,2,\dots,n\}$. We consider distribution
...
c_I(X_m)$ be the target labels. Let $X$ be $m \times n$ matrix with entries

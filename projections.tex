...
Consider $k \in \{1,2,\dots,n\} \setminus \{i,j\}$. Then
\begin{align*}
d_{P_i}(c_j, c_k)
& = \Pr_{X \sim P_i}[c_j(X) \neq c_k(X)] \\
& = \Pr_{X \sim P_i}[c_j(X) = 1 \wedge c_k(X) = 0] + \Pr_{X \sim P_i}[c_j(X) = 0 \wedge c_k(X) = 1] \\
& = \Pr_{X \sim P_i}[X[j] = 1 \wedge X[k] = 0] + \Pr_{X \sim P_i}[X[j] = 0 \wedge X[k] = 1] \\
...
For any distribution-independent algorithm, any $\epsilon \in (0,\frac{1}{2})$ and any
...
$$
\Pr \left[ d_{P_i}(c_i, A(X,Y)) > \frac{1}{16} \right] \ge \frac{\frac{1}{8} - \frac{1}{16}}{1 - \frac{1}{16}} > \frac{1}{16} \; .
$$
...
& =
\begin{cases}
\frac{1}{2} \epsilon^{\norm{z}_1 - 1} (1 - \epsilon)^{n - \norm{z}_1} & \text{if $z[i] = 1$,} \\
\frac{1}{2} \epsilon^{\norm{z}_1} (1 - \epsilon)^{n-1 - \norm{z}_1}& \text{if $z[i] = 0$.} \\
\end{cases}
\end{align*}
...
& \quad = \frac{1}{2|k(x,y)|} \cdot |s(x,y,z)| \cdot \epsilon^{\norm{z}_1 - 1} (1 - \epsilon)^{n - \norm{z}_1} + \frac{1}{2|k(x,y)|} \cdot (|k(x,y)| - |s(x,y,z)|) \cdot \epsilon^{\norm{z}_1} (1 - \epsilon)^{n - 1 - \norm{z}_1} \\
& \quad = \frac{\epsilon^{\norm{z}_1 - 1} (1 - \epsilon)^{n - 1 - \norm{z}_1}}{2|k(x,y)|} \cdot \left( |s(x,y,z)| \cdot (1 - 2\epsilon) + |k(x,y)| \cdot \epsilon \right) \; .
\end{align*}
Hence,
\begin{align*}
...
& \quad = \frac{\displaystyle \frac{|s(x,y,z)|}{2|k(x,y)|} \cdot \epsilon^{\norm{z}_1 - 1} (1 - \epsilon)^{n - \norm{z}_1}}{\displaystyle \frac{\epsilon^{\norm{z}_1 - 1} (1 - \epsilon)^{n - 1 - \norm{z}_1}}{2|k(x,y)|} \cdot \left( |s(x,y,z)| \cdot (1 - 2\epsilon) + |k(x,y,z)| \cdot \epsilon \right) } \\
...
\begin{equation}
\frac{|k(x,y)| \cdot \epsilon}{|s(x,y,z)|} \in \left[\frac{5}{6}, 2 \right]
\end{equation}
then since $\epsilon \in (0,\frac{1}{4})$,
$$
...
& = \sum_{\substack{x \in \{0,1\}^{m \times n} \\ y \in \{0,1\}^m \\ z \in \{0,1\}^n}} \frac{1}{2} - \left| \frac{1}{2} - \frac{\displaystyle 1 - \epsilon}{\displaystyle 1 - 2\epsilon + \frac{|k(x,y)| \cdot \epsilon}{|s(x,y,z)|}} \right| \cdot \Pr \left[X = x, Y = y, Z = z \right] \\
& \ge
